...
//
// Une arme qui ne tire pas est ce que demandent les cas éprouvant la mort ou le
// déplacement : leurs cobayes ne doivent pas être abattus en cours de route. Les
// passifs, eux, restent ceux du fichier — le monde en a besoin dès le montage,
// et une table forgée ferait diverger ce que le test voit de ce que le jeu
// offre.
func armesInertes(t *testing.T) *Weapons {
	t.Helper()
	armes, err := LoadWeapons(cohue.Assets, manifesteArmes)
	if err != nil {
		t.Fatalf("armes livrées : %v", err)
...
func TestRienNeTraverseUnMur(t *testing.T) {
	...
	w := NewWorld(profils, armesInertes(t), progressionLivree(t), sansVagues(), g, graineDeTest, 4, 1, 4, 8)
	...
	for _, c := range cas {
		t.Run(c.nom, func(t *testing.T) {
			...
			w := NewWorld(profils, armesInertes(t), progressionLivree(t), sansVagues(),
				grilleDepuis(c.grille...), graineDeTest, 1, 1, 1, 4)
...
func TestLeCoutDeLaCaseDiviseLaVitesse(t *testing.T) {
	...
	parcours := func(cout Cost) Fixed {
		...
		w := NewWorld(profils, armesInertes(t), progressionLivree(t), sansVagues(), g, graineDeTest, 1, 1, 1, 4)
